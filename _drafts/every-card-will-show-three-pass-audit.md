...
The new subheads keep the model failure, corrected same-day representation, history transition, and implementation bridge from reading as one uninterrupted proof tutorial. No section is left doing multiple unrelated jobs after this pass.
...
| Public code excerpts match the current Lean source | `Card`, `ImpressionHistory`, and `simulateDayFrom` from `lean/LotLead/Cap.lean`; `LocalVisit`, `fiveVisitsOn`, `freshHistoryFor`, and the displayed theorem from `lean/LotLead/Cover.lean` at `fffec3548` | Kept the examples adjacent to their plain-language translations and updated the discussion for the evolving history. |
| Lean checks coverage from two selected uniform starting histories | The all-unseen and all-fresh theorems in `lean/LotLead/Cover.lean` | Corrected `history extremes` to two explicit cases: no prior impressions, and exactly one impression immediately before 8 AM. The post now says this does not quantify over arbitrary counts or ages, mixed histories, changing eligibility, changing modes, or cards retained but not actually viewed. |
...
- Disclosed the fixed empty starting history and fixed event-lead/tour modes in the coverage theorem.
