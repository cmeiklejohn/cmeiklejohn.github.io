...
- Blog `origin/master` at `5f67103419df53582275f76c809445c3367e69fd`.
...
| Public code excerpts match the current Lean source | `ownsProgramPosture` and `takePhase0` from `lean/LotLead/Cap.lean`; `programmed_capacities_match_catalog`, `covered`, and `missing` from `lean/LotLead/Cover.lean` at `51995c943` | Kept the examples adjacent to their plain-language translations and centered them on the reservation-and-capacity mechanism. |
...
- Identified the stronger theorem the design suggests, coverage under arbitrary ranking, while stating that the current Lean source performs a finite full-selector check instead of proving that generic lemma.
...
- Shortened the remaining Lean walkthrough to the five definitions that bear directly on starvation: program ownership, reservation-first selection, assignment counts, coverage, and missing cards. Moved the production selector's extra complexity into the differential-testing bridge.
