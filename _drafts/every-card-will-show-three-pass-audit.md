...
| Public code excerpt matches the current Lean source | `modeledCardNames`, `cardsAcrossFivePrograms`, and `union_of_five_programs_is_all_modeled_cards` from `lean/LotLead/Starvation.lean` | The only displayed theorem says that the union across five programs equals the modeled catalog. `nameSet` is explained as sorting and deduplicating the two name lists. |
