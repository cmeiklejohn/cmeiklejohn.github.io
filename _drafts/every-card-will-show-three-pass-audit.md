...
6. Lean primer and chronology: keep as the final technical section, split under `Lean proved the wrong day`, `The proof was still too complicated`, `The theorem we actually needed`, and `Connecting Lean back to Go`. The added middle turn records that author review, not the agent, identified that `localDay`, selected histories, modes, scoring, and a raw capacity vector obscured a much smaller invariant. Distinguish the direct set-equality theorem from the more detailed executable twin used for differential comparison.
...
- Replaced the indirect empty-starvation theorem with direct equality between the set of cards across the five programs and the set of available cards. Capacity remains a separate supporting theorem.
...
- Shortened the Lean walkthrough to `availableCards`, `cardsAcrossFivePrograms`, and their equality theorem. Moved capacity and the production selector's extra complexity into prose and the differential-testing bridge.
