...
6. Lean primer and chronology: keep as the final technical section, split under `Lean proved the wrong day`, `The proof was still too complicated`, `The theorem we actually needed`, and `Connecting Lean back to Go`. The added middle turn records the discovery that `localDay`, selected histories, modes, scoring, and a raw capacity vector obscured a much smaller invariant. Distinguish the direct set-equality theorem from the more detailed executable twin used for differential comparison.
...
| Current program capacities fit their reservation counts | `LotLead.Cap.capForDaypart`, `Starvation.reservationsFit`, and the successful current proof report | Removed the raw five-number vector from the public explanation. Capacity is a separate supporting check, not the coverage theorem. |
...
| Public code excerpts match the current Lean source | `modeledCardNames`, `cardsAcrossFivePrograms`, `union_of_five_programs_is_all_modeled_cards`, and `every_fixed_eligible_subset_has_an_opportunity` from `lean/LotLead/Starvation.lean` | The first theorem says that the union across five programs equals the modeled catalog. The second intersects both equal sets with one fixed eligibility set. |
...
| `modeledCardNames` denotes the full modeled catalog, not dynamic runtime eligibility | `Catalog.fullNames`, `Starvation.modeledCardNames`, and the `allApplicableRankedLotCards` Go-test assumption | Renamed the definition and explained the assumption before the code excerpt so readers do not interpret the theorem as quantifying over arbitrary live eligibility changes. |
| Every fixed eligible subset inherits the full-catalog opportunity and capacity guarantees | `every_fixed_eligible_subset_has_an_opportunity` and `every_fixed_eligible_subset_fits_program_capacity` in `Starvation.lean` | Added the general subset theorem and its capacity companion. Explicitly bounded both to one eligibility set held fixed across the five visits. |
| The unrestricted changing-eligibility guarantee is false | `a_card_becoming_eligible_after_its_reservation_can_be_starved` and `becomesEligibleAfterMorningKeeps` in `Cover.lean` | Added the checked counterexample where **Last Night** is absent during its reserved morning program, becomes applicable for the other four visits, and remains the exact missing card. |
...
- Replaced the indirect empty-starvation theorem with direct equality between the set of cards across the five programs and the modeled catalog. Capacity remains a separate supporting theorem.
- Added the author-review discovery to the public chronology: the AI-written repair was valid but unnecessarily mirrored production state, and the author identified the smaller theorem by questioning each input while editing the post.
- Removed the raw five-capacity vector and the `reservationsFit` code block from the public explanation so capacity no longer looks like the definition of coverage.
...
- Recentered the Lean explanation on one direct set equality. Reservation capacity and the production `takePhase0` implementation remain in the separate implementation bridge rather than the proof excerpt.
...
- Split the Lean material into four named transitions so a reader can follow the wrong proof, the still-overcomplicated repair, the direct theorem, and the implementation bridge.
...
- Shortened the Lean walkthrough to `modeledCardNames`, `cardsAcrossFivePrograms`, and their equality theorem. Moved capacity and the production selector's extra complexity into prose and the differential-testing bridge.
- Rechecked the displayed theorem against merged PR #2651, rendered the revised section, and confirmed that no stale date, hour, history, selector, or raw-capacity code remains in the excerpt.
