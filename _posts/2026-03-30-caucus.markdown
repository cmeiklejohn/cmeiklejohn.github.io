...
I don't say this to diminish their work. These systems made real breakthroughs on the agent layer — role specialization, structured dialogue, tool use patterns, task decomposition. The agents themselves are impressive. But the coordination layer underneath them is exactly the problem space that distributed systems research has been working on for decades, and I kept recognizing failure modes I'd seen before.

## We've Seen This Before

The reason I recognize these problems is that I spent a decade working on them in a different context.
...
The thread connecting all of this work is a single question: how do independent processes coordinate in the presence of partial failure?

## Why This Is Inevitable

Here's the thing people miss about multi-agent systems: the distributed systems problems aren't a bug. They're not an artifact of bad architecture or missing features. They're an inevitable consequence of having multiple autonomous processes that share state. Every multi-agent system, no matter how it's built, will hit the same categories of failure.
...
What's interesting to me is that these are all problems with known solutions. Distributed systems research has spent fifty years developing formal models for concurrent access, data structures that merge automatically, and techniques for systematically testing every failure mode. None of this has made it into the multi-agent stack yet.

The question I keep coming back to is whether the solutions transfer. Lasp showed that if you build on CRDTs, convergence comes for free — but LLM agents aren't database replicas. They hallucinate. They lose context. They make confident decisions based on incomplete information. Partisan and Filibuster showed you can systematically verify coordination by injecting every failure you can think of — but the failure modes for agents (context overflow, hallucinated fixes, contradictory beliefs) are different from network partitions and crashed processes. The structural parallels are strong, but the open research question is whether the techniques actually carry over, and what has to change when they do.

*In a [follow-up post](/ai/agents/distributed/zabriskie/2026/04/03/caucus-system-design.html), I dig into specific experiments: what happens when you actually try to apply CRDTs, vector clocks, and fault injection to a multi-agent system?*
