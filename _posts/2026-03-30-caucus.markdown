...
A few weeks ago I was sitting in the Beacon Theatre watching Tedeschi Trucks Band and fixing bugs in Zabriskie from my phone. I've [written about this before](/ai/zabriskie/development/2026/03/29/the-show-is-happening-right-now-and-nothing-works.html) — about what it's like to build a social app with Claude as your only collaborator, deploying CSS fixes from your seat while Derek Trucks plays. One human, one agent, one codebase. It works.

But I kept thinking about what happens when it's not just one agent. I'm hitting problems now that a single agent can't solve efficiently — bugs from real users coming in while I'm trying to ship features, tests that need writing, infrastructure that needs maintaining, all at the same time. The obvious answer is more agents. A team. And the moment you have multiple agents working on the same codebase — reading the same files, writing to the same database, making decisions based on shared state — something clicked for me.

I watched two Claude Code instances step on each other's migrations last month. One created migration 267. The other, running in a different worktree, also created migration 267. Different schemas, same filename. The second one silently overwrote the first. I stared at it for a minute before I started laughing. This is a lost update. The exact same problem that distributed databases have been solving since the 1970s. It's Lamport's [happened-before relation](https://lamport.azurewebsites.net/pubs/time-clocks.pdf), playing out in a directory of SQL files instead of a network of processes. This is the kind of problem [CRDTs](https://hal.inria.fr/inria-00555588) were invented to solve: two independent writers, no coordination, and the system needs to merge their work without losing either update.
...
I spent my PhD working on [Lasp](https://arxiv.org/abs/1510.07191), which tried to take the CRDT thinking a step further: instead of just using CRDTs as data structures, Lasp made them the basis for coordination-free distributed programming. Programs in Lasp computed over CRDTs directly — maps, filters, folds — so the entire application inherited their convergence guarantees. As part of SyncFree's partnership with [Rovio Entertainment](https://www.rovio.com/), we demonstrated that CRDTs could be used at scale in controlled experiments with real outcomes, running Lasp on over 1,000 nodes on AWS — at the time, one of the largest CRDT deployments in academic research. It ended up in LoJack's stolen car recovery service across South America. That work received the PPDP 10-year most influential paper award last year.
...
What fascinates me is that these are all problems with known solutions — or at least, known solutions in the distributed systems world. Fifty years of research on formal models for concurrent access, data structures that merge automatically, techniques for systematically testing every failure mode. None of this has made it into the multi-agent stack yet. And I'm genuinely not sure how much of it transfers cleanly.

Lasp showed that if you build on CRDTs, convergence comes for free — but LLM agents aren't database replicas. They hallucinate. They lose context. They make confident decisions based on incomplete information. Partisan and Filibuster showed you can systematically verify coordination by injecting every failure you can think of — but the failure modes for agents (context overflow, hallucinated fixes, contradictory beliefs) are strange new versions of network partitions and crashed processes. The structural parallels are strong. Whether the techniques actually carry over, and what has to change when they do — that's the part I find genuinely exciting to think about.
