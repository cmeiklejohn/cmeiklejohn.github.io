...
With the agents I use now, I believe I could produce most of that instrumentation and test scaffolding in days, perhaps a single day. I cannot rerun my PhD as a controlled comparison, so this is a counterfactual estimate rather than a measured speedup. And code arriving quickly would not establish that the instrumentation preserved the application's behavior or that the generated tests supported the evaluation. Those questions would still require evidence. But compressing nine months of prerequisite engineering into days changes which research projects one person can plausibly attempt.
