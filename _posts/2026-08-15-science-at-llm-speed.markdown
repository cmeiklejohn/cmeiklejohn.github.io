...
With the agents I use now, I believe I could produce a first implementation of much of that instrumentation and test scaffolding in days, perhaps a single day. That estimate comes from the speed with which I can now build comparable implementation scaffolds, but I cannot rerun my PhD as a controlled comparison. It is a counterfactual, not a measured speedup. Code arriving quickly would not establish that the instrumentation preserved the application's behavior or that the generated tests supported the evaluation. Those questions would still require evidence. But even compressing the first implementation from months into days changes which research projects one person can plausibly attempt.
...
While I was writing this, Jeff Dean, Sanjay Ghemawat, Oriol Vinyals, and Quoc Le left Google to form a company called [Discovery Loop](https://www.axios.com/2026/08/06/googles-ai-leadership-shuffle). Public descriptions say its goal is to [automate experimental loops](https://www.itpro.com/business/leadership/deepmind-ceo-demis-hassabis-steps-aside-amid-google-leadership-shake-up) in science and engineering: propose an experiment, run it, evaluate what happened, and use the result to decide what to try next. The details of their system are not public. What caught my attention was that four of Google's most prominent AI researchers were starting a company to build the same thing I had been trying to build on my own: a human-out-of-the-loop research system that could propose an experiment, implement it, run it, evaluate the result, and decide what to try next without waiting for a person between each step.

I had been trying to build a much smaller discovery loop of my own. I run Zabriskie, a community for live-music fans. During a livestream by the band Goose, viewers who want a running setlist have to wait for a person or an external service to recognize each song and enter it. I wanted Zabriskie to infer the setlist from the audio itself: listen to the stream, identify the current song, and post the guess while the show was still happening. That recognizer became SetScope.

SetScope is obviously not operating at the scale of that company's ambitions. Goose song identification is probably not its launch domain. But the objective of the research system around it is the same. I wanted to specify the question and the constraints, then let the system carry the work around the experimental loop without requiring me to approve every transition. Once an agent can propose an experiment, implement it, run it, read the result, and modify the next experiment, research velocity becomes a property of the loop rather than any one model response. So do the failures. A contaminated result can alter the next hypothesis. A convenient proxy can become the objective. A component test can be promoted into a claim about a system that never ran.
...
The agent produced code that ran, clean-looking metrics, detailed figures, interpretations, two long articles, and a listening study. I had required performance-disjoint training and test data, but I accepted the split report without checking whether different encodings, provider copies, or track cuts from the same performance had crossed the boundary. On the restart, I had explicitly forbidden a fixed 90-second boundary for improvisation, but I did not inspect the value the implementation actually used. The immediate errors were in generated code. Treating the resulting artifacts as progress was my mistake.

In the first notebook, audio from the same recorded performances appeared in both training and test under different files. I deleted it when I found the leakage and started again. In the second notebook, a 90-second default that the written method had explicitly warned against silently defined which parts of a track counted as improvisation. I deleted that restart after finding the segmentation error. Both mistakes were small in code. Both propagated through almost everything that followed.

We eventually returned to the original question: could SetScope identify a song from a live stream soon enough to update a setlist? On August 13, its runtime emitted correct identities for 10 of 12 song performances while the band was playing. It also missed songs, switched guesses at the wrong time, and exposed failures in the live audio path. This was a product field test, not a formal whole-show accuracy estimate, and the complete operational record belongs later in this series. Neither audio from that performance nor its completed setlist existed during development. SetScope timestamped each guess before the final setlist was known.
