...
I am a PhD-trained systems researcher. I know what a holdout is. I know why an experimental method has to match its implementation. The first notebook began with an explicit requirement that the same recorded performance could not appear in both training and test. I still accepted the split report without checking whether different encodings, provider copies, or track cuts of one performance had crossed the boundary.
