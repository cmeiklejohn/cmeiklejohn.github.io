...
I stared at it for a minute before I started laughing. This is a lost update — the exact same problem that distributed databases have been solving since the 1970s. A lost update playing out in a directory of SQL files instead of a network of processes. The kind of problem [CRDTs](https://hal.inria.fr/inria-00555588) were invented to solve: two independent writers, no coordination, and the system needs to merge their work without losing either update.

I've been [building an app with Claude Code](/ai/zabriskie/development/2026/03/29/the-show-is-happening-right-now-and-nothing-works.html) as my only collaborator for about two months now. One human, one agent, one codebase — it works. But I'm hitting problems now that a single agent can't solve efficiently: bugs from real users coming in while I'm trying to ship features, tests that need writing, infrastructure that needs maintaining, all at the same time. The obvious answer is more agents. And the moment you have multiple agents working on the same codebase, you have a distributed system.
...
Once I started looking, I saw the same gaps everywhere. Take the [ChatDev paper](https://arxiv.org/abs/2307.07924).
...
- **Byzantine faults.** In distributed systems, a [Byzantine fault](https://lamport.azurewebsites.net/pubs/byz.pdf) is a process that doesn't just crash — it produces incorrect output while appearing to function normally. LLM agents do this constantly. An agent hallucinates a fix that looks plausible, passes its own tests, and ships it. A downstream agent reads that fix, trusts it, and builds on top of it. Now you have a chain of work built on a foundation that was wrong from the start. In traditional distributed systems, Byzantine fault tolerance is the hardest problem — it requires redundancy, voting, verification. In multi-agent systems, every agent is a potential Byzantine actor every time it responds.
