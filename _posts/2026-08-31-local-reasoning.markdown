...
Human codebases rot this way too. The difference is what happens to the person who wrote the code. You ship a slow page, you watch it crawl, and you build the next one differently. You get paged at 2am and the lesson sticks. Nobody in this loop gets paged. This codebase took a couple thousand pull requests in a few months, each one from a session that had never seen the app before and was gone before the code ran. A session knows only what's written down for it, and nothing written down said what a request should cost. I never thought to write it down either.
...
The change isn't banning agents from the backend. Look at what actually failed: how many connections a request holds at once, how many queries a page runs, how much of the database's time one function eats. Every one of those is a total across the whole program, and nothing in the loop ever checks a total. The build checks one change, the tests check one behavior, a review checks one diff. All of those passed, all summer, while the totals grew. So the change is boring: every total the app depends on gets a number and a test that fails when the number is exceeded.

Statements per request is first, because it's measurable now. The next step is a budget in CI: every page gets a ceiling, and the build fails the day the page crosses it. The fix for the nine-queries comment is the same idea, make it a number in the file with a test attached, instead of prose. The pool's wait counters are logging now, so the next pool problem shows up in the logs instead of behind a green health check. And a job runs every day that reads production's query statistics and looks for the next 79.5 percent, because every fix in this story was cheap once the problem was visible, one keyword, two indexes, a rewritten loop. The expensive part, every time, was noticing.

None of this catches everything. A query budget wouldn't have caught the outage: the gridlock was about holding two connections at once, not about how many queries ran, and the wait counters are what cover that now. What would actually answer "does this survive a crowd" is a load test, fake traffic pointed at the app before a real crowd arrives, and I still haven't run one. There's also still no connection pooler in front of Postgres. Both belong on the list.

The Shows page still runs 340 statements per request, after everything. Does it survive the next crowd? I don't know, and neither does anyone else, because nobody has pointed one at it. On a human team, somebody eventually asks that question without being told to, usually the engineer who's been burned before. I spent years being that person for other people's systems. The question this summer leaves me with is whether an agent loop can be made to ask it, because the models were never the problem. Everything in this post fit inside a context window. Asking was nobody's job.
