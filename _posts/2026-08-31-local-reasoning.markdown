...
The obvious objection is that human codebases rot exactly this way too, and they do. Nothing about an LLM invented the chatty page. The difference is what happens after the rot ships. An engineer who ships a slow page and then watches it crawl in production builds their next page differently. On-call turns production behavior into instinct. Ownership keeps one person attached to a surface long enough to notice its sum drifting. All of those channels work because the same person is still around when the consequences land, and an agent loop has no such person. This codebase absorbed a couple thousand pull requests in a matter of months, each one written by a session that had never seen the app before, will never see it again, and never waits on a page it built. What a session should know about cost has to be written down where the next session will read it, and nothing anyone wrote down said what a request should cost. It never occurred to me either.

To be fair to the agents, the same loop that never noticed the home page running 292 statements diagnosed the gridlock while the site was down, threw out its own bad scan before acting on it, and reverted six of its own eight optimizations when the benchmarks came back against them. With the evidence in front of them, these agents matched any engineer I've worked with, at several times the speed. Diagnosis is a local task. The evidence shows up already gathered, and the mechanism is right there in the fragment. Prevention is the global one, and nothing in the loop was ever charged with a global. Me included. The nine-queries comment crossed my desk inside a diff and I approved it. I've come to think that careful cost prose in a diff lowers scrutiny rather than raising it. It reads as diligence, nothing fires when the price stops being true, and the diligence is exactly what makes you stop checking.

## Make every global a local number

What I'm changing isn't "stop letting agents write the backend." Local reasoning isn't the defect. It's the only reasoning anyone gets, agent or human; nobody holds a whole program in their head. Local reasoning is safe when every global it leans on is held still by something, a guarantee, a budget, an alarm, and it's a trap when a global is held by nobody. Everything that failed this summer was the trap. Connection hold time, statements per request, share of database time: properties of the whole program at once, and nobody's job. So the change is to make every global this app depends on into a local number that some check can see and fail on.

Statements per request is measurable now, so the next step is a budget in CI, a test that fails the day a page crosses its ceiling. The card renderer that runs sixty-five times a page should say so in the file, where the next session will trip over it, welded to that budget so it can't rot into a lie the way "about nine queries" did. The looking itself has to be somebody's job too. Every repair in this story was trivial once the defect was seen, one keyword, two indexes, a rewritten loop. When fixing is nearly free, discovery is the entire cost, which is why there's now a daily job that walks production's query statistics and just looks.

I'll be honest about the limits. A statement budget wouldn't have caught the gridlock. That failure was about how connections were held, not how many statements ran; the pool's wait counters are what catch it now. Catching it before a crowd instead of during one takes the load test I still haven't run, and a real connection pooler in front of Postgres, which still isn't there. The census doesn't replace either.

The Shows page runs 340 statements per request today, after the sweep, behind the guardrails. Nobody can say whether it survives a crowd, because nobody has pointed one at it. What I have that I didn't have in August is the number. What I still don't have is whatever names the next global, the way connection hold time was August's and statements per request was this week's. In a human organization that's somebody's job, the engineer who wonders, unprompted, what happens when this meets a crowd. I did that job for years, in other people's systems. Whether an agent loop can be made to do that wondering is the question this summer leaves me with, and it isn't a question about smarter models. Every model in this story was smart enough. The sum always fit in a context window. It was never anyone's assignment.
